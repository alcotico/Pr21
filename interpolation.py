...
class Intpl1D:
    ...
    cubical(n: int) - возвращает кусочно-кубическую интерполяцию данных;
    lagrange(n: int) - возвращает интерполяцию данных многочленом Лагранжа;
    cubical_smooth(n: int, derivs: str = 'z') - возвращает интерполяцию данных кубическими полиномами с дополнительными
    условиями в виде значений производных в узлах;
    ...
    def cubical_smooth(self, n: int, derivs: str = 'z'):
        """
        Производит продвинутую интерполяцию кубическими полиномами.
        :param n: количество точек дискретизации одного отрезка, включая его граничные точки (значение не может быть меньше 3).
        :param derivs: способ определения массива производных функции в узлах данных;
        z - означает, что производные в узлах будут равны 0, с - будут построенны окружности по двум ближайшим точкам,
        и взяты их производные в соответствующей точке.
        :return: структуру вида Tuple[Tuple[x0, y0], Tuple[x1, y1], ... ], дискретно описывающую полученную функцию интерполяции.
        """
        if derivs not in ['z', 'c']:
            raise KeyError('вид поиска производных в узлах должен задаваться ключами z или c')
        ...
        # переменная @der отвечает за хранение производных узлов функции
        if derivs == 'z':
            der = [0 for i in range(points)]
            ...
            for i in range(1, points - 1):
                ...
                # Уравнение окружности имеет вид (x-a)^2 + (y-b)^2 = r^2 (причем, поскольку r^2 это  в общем случае
                # число, его можно обозначить за R, чтобы не обращать внимания на квадрат). Решая систему из трех
                # уравнений, где вместо аргумента и значения функции используются координаты трех точек, можно получить
                # нижеприведенные формулы для рассчета параметров а, b и R. Более того, функция является неявной,
                # поэтому и ее производная берется как производная неявной функции.
        ...
        for i in range(points - 1):
            ...
            # из системы с 4 уравнениями можно получить нижеприведенные формулы и по ним, соответственно, расчитать
            # коэффициенты кубических полиномов, которые учитывают, какие производные должны быть в узлах функции
            d = (der[i + 1] + der[i]) / (x1 - x0) ** 2 - 2 * (y1 - y0) / (x1 - x0) ** 3
            c = (der[i] - (y1 - y0) / (x1 - x0) + d * (x1 ** 2 + x1 * x0 - 2 * x0 ** 2)) / (x0 - x1)
